...
object BottomResizableInteractor: ShapeInteractor {

    override fun detectInteraction(shape: ShapeData, x: Float, y: Float): ShapeData {
        return if (x in (shape.startX + TOUCHABLE_WIDTH/2 .. shape.endX - TOUCHABLE_WIDTH/2) &&
                y in (shape.endY - TOUCHABLE_WIDTH/2 .. shape.endY + TOUCHABLE_WIDTH/2)) {
            shape.copy(activeInteractionMode = InteractionMode.RESIZE_BOTTOM)
        } else {
            shape
        }
    }
